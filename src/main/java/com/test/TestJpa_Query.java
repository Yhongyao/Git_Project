...
//		TestJpa_Query.pageSQL();
	...
	private static void basicQuery() {
		...
		try {
			...
			//4、加入查询条件
			jpql = "Select u From UserBean u where username like '%a%' and truename like '%韦%' order by userid asc";
...
//						query.setParameter(2, "%韦%");
	...
	private static void singleResult() {
		...
		try {
			...
			tx.begin();
			//1、查找所有的字段
			jpql = "Select u From UserBean u where username = 'jokic'";
			Query query = manager.createQuery(jpql);
			UserBean userBean = (UserBean) query.getSingleResult();
			System.out.println(userBean);

			//2、返回部分的字段
			jpql = "Select userid,username,password From UserBean where username = 'jokic'";
			...
			//4、只返回一列的具体类型
			jpql = "Select truename From UserBean where username = 'jokic'";
			query = manager.createQuery(jpql);
	...
	private static void dmlSQL() {
		...
		try {
			...
			//delete语句
			jpql = "Delete From UserBean where userid in (17,18,19)";
			query = manager.createQuery(jpql);
